...
def run_one_model_without_prune(random_state, x, y, device):
    ...
    model = KAN(width=[2000, [5, 2], 4], grid=3, k=3, device=device, grid_range=[-10, 10])
    ...
    results = model.fit(dataset, opt="LBFGS", steps=10, metrics=(train_acc, test_acc, train_auc, test_auc),
                        loss_fn=torch.nn.CrossEntropyLoss(), lr=1, lamb=0.002, lamb_entropy=10, lamb_coef=1, update_grid=True)
    # results = model.fit(dataset, opt="Adam", steps=500, metrics=(train_acc, test_acc),
    #                    loss_fn=torch.nn.CrossEntropyLoss(), update_grid=False, singularity_avoiding=True,
