...
def run_one_model(random_state, x, y, device, prune_remain_num=30):
    ...
    model = KAN(width=[2000, 5, 4], grid=8, k=3, device=device, grid_range=[-10, 10])
    ...
    results = model.fit(dataset, opt="LBFGS", steps=10, metrics=(train_acc, test_acc, train_auc, test_auc),
                        loss_fn=torch.nn.CrossEntropyLoss(), lr=1, lamb=0.002, lamb_entropy=10, lamb_coef=1)
    # results= model.fit(dataset, opt="Adam", steps=50, metrics=(train_acc, test_acc), loss_fn=torch.nn.CrossEntropyLoss(),
    #                    lr=1e-3, lamb=0.002, lamb_entropy=10, lamb_coef=1)
    ...
    # Retrain the model
    # model.fit(dataset, opt="Adam", steps=2000, metrics=(train_acc, test_acc), loss_fn=torch.nn.CrossEntropyLoss(),
    #          lr=1e-2, lamb=0.001, update_grid=False, singularity_avoiding=True)
    # Retrain the model when remain 1,2 input knots, avoid singularity, get rid of L1 reg
    results_prune = model.fit(dataset, opt="Adam", steps=50, metrics=(train_acc, test_acc), loss_fn=torch.nn.CrossEntropyLoss(),
                    lr=1e-3, lamb=0.000, update_grid=False, singularity_avoiding=True)
    ...
    return (
        ...
        (results_prune['train_loss'], results_prune['test_loss']),
    )
